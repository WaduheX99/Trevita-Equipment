...
import dao.daoDetailPemesanan;
import dao.daoLaporan;
import dao.daoPemesanan;
import dao.daoTempPemesanan;
...
import tableModel.tableModel_DetailPemesanan;
import tableModel.tableModel_Pemesanan;
import tableModel.tableModel_TempPemesanan;


public class Transaksi_Pemesanan extends javax.swing.JPanel {

    private servicePemesanan servis_psn = new daoPemesanan();
    private serviceDetailPemesanan servis_det = new daoDetailPemesanan();
    private serviceTempPemesanan servis_sem = new daoTempPemesanan();
    
    private tableModel_Pemesanan tblModel_psn = new tableModel_Pemesanan();
    private tableModel_DetailPemesanan tblModel_det = new tableModel_DetailPemesanan();
    private tableModel_TempPemesanan tblModel_temp = new tableModel_TempPemesanan();
    ...
    public Transaksi_Pemesanan(String Id) {
        ...
        tbl_temporary.setModel(tblModel_temp);
        //setColumnWidth();
        loadData();
        loadDataSementara();
...
//    }
    ...
    private void btn_tambahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_tambahActionPerformed
        ...
        lb_total.setText(det_psn.getSubtotal_pemesanan().toString());
        setTanggal();
    ...
    private void tbl_barangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_barangMouseClicked
        ...
        List<detailPemesananModel> list = servis_det.getData(id);
...
//            servis_lap.suratJalanPemesanan(no);
    ...
    private void btn_tambahSementaraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_tambahSementaraActionPerformed
        ...
        det_psn.setJml_pemesanan(jumlah);
        det_psn.setSubtotal_pemesanan(subtotal);
        det_psn.setStatus(status);
        //psn.setTotal_pemesanan(tobar);
    ...
    private void btn_hapusSementaraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_hapusSementaraActionPerformed
        ...
        if (index!=-1) {
            tempPemesananModel model = tblModel_temp.getData(tbl_temporary.convertRowIndexToModel(index));
            if (JOptionPane.showConfirmDialog(null, "Yakin data akan dihapus?","Konfirmasi", JOptionPane.OK_CANCEL_OPTION)== JOptionPane.OK_OPTION) {
                ...
                tblModel_temp.deleteData(index);
                loadDataSementara();
                resetForm();
                bersihBarang();
...
//            t_stok.setText              (tbl_barang.getModel().getValueAt(row, 7).toString());
    ...
    private void loadData() {
        ...
        List<pemesananModel> list = servis_psn.getData();
        tblModel_psn.setData(list);
    }

    private void loadDataSementara() {
        List<tempPemesananModel> list = servis_sem.getData();
        tblModel_temp.setData(list);
    }
    
    private void hapusData() {
        int index = tbl_barang.getSelectedRow();
        if (index!=-1) {
            pemesananModel brg = tblModel_psn.getData(tbl_barang.convertRowIndexToModel(index));
            if (JOptionPane.showConfirmDialog(null, "Yakin data akan dihapus?","Konfirmasi", JOptionPane.OK_CANCEL_OPTION)== JOptionPane.OK_OPTION) {
                servis_psn.deleteData(brg);
                tblModel_psn.deleteData(index);
                loadData();
                resetForm();
                pn_detail.setVisible(false);
            }
    ...
    private void simpanData() {    
        String no_pemesanan   = t_noPesan.getText();
        String tgl_pemesanan   = t_tanggalPesan.getText();
        String kode = t_kodeBarang.getText();
        String nama = t_namaBarang.getText();
        ...
        barangModel brg = new barangModel();
        pemesananModel psn = new pemesananModel();
        detailPemesananModel det = new detailPemesananModel();

        //Simpan Tabel Pemesanan
        psn.setNomor_pemesanan(no_pemesanan);
        psn.setTgl_pemesanan(tgl_pemesanan);
        psn.setTotal_pemesanan(total);
    ...
    private void updateDataTemp() {
        ...
        if (index!=-1) {
            tempPemesananModel model = tblModel_temp.getData(tbl_temporary.convertRowIndexToModel(index));
                ...
                barangModel brg = new barangModel();
                detailPemesananModel det = new detailPemesananModel();
                tempPemesananModel tem = new tempPemesananModel();

                brg.setKode_barang(kode);
                brg.setNama_barang(nama);
                brg.setHarga(harga);
                det.setJml_pemesanan(jumlah);
                det.setSubtotal_pemesanan(subtotal);

                tem.setBarang_Model(brg);
                tem.setDetailPemesanan_Model(det);

                servis_sem.updateData(tem);
                tblModel_temp.updateData(index, tem);
                servis_det.sumTotal(det);
                lb_total.setText(det.getSubtotal_pemesanan().toString());
                loadDataSementara();
                bersihBarang();
                resetForm();
    ...
    private void search() {
        List<pemesananModel> list = servis_psn.search(t_cari.getText());
        tblModel_psn.setData(list);
    }
}