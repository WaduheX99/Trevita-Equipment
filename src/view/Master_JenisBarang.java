...
import tableModel.tableModel_JenisBarang;


public class Master_JenisBarang extends javax.swing.JPanel {

    private serviceJenisBarang servis = new daoJenisBarang();
    private tableModel_JenisBarang tblModel = new tableModel_JenisBarang();
    ...
    private void loadData() {
        ...
        List<jenisBarangModel> list = servis.getData();
    ...
    private void hapusData() {
        ...
        if (index!=-1) {
            jenisBarangModel brg = tblModel.getData(tbl_data.convertRowIndexToModel(index));
    ...
    private void simpanData() {
        ...
        jenisBarangModel jb = new jenisBarangModel();
        ...
        if(servis.validasiNamaJenisBarang(jb)==true)
        {
            if(validasiInput()==true){
                ...
                jenisBarangModel jbg = new jenisBarangModel();
    ...
    private void perbaruiData() {
        int index = tbl_data.getSelectedRow();
        if (index!=-1) {
            jenisBarangModel mobar = tblModel.getData(tbl_data.convertRowIndexToModel(index));
            ...
            if(servis.validasiNamaJenisBarang(jb)==true)
            {
                if(validasiInput()==true){
                    ...
                    servis.updateData(jbr);
                    tblModel.updateData(index, jbr);
                    loadData();
                    resetForm();
                    tampilPanel();
    ...
    private void pencarian() {
        List<jenisBarangModel> list = servis.search(t_cari.getText());
        tblModel.setData(list);
    }
}
